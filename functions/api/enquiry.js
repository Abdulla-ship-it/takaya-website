...
export async function onRequestPost({ request, env }) {
  ...
  if (env.RESEND_API_KEY) {
    ...
    const rows = [
      ...
      .filter(([, v]) => v)
      .map(([k, v]) => `<tr><td style="padding:4px 14px 4px 0;color:#3E5372">${k}</td><td style="padding:4px 0"><strong>${escapeHtml(v)}</strong></td></tr>`)
      .join('');
        ...
        body: JSON.stringify({
          ...
          html: `
            <div style="font-family:system-ui,-apple-system,sans-serif;color:#3E5372;max-width:560px">
              ...
              ${enquiry.message ? `<p style="margin:22px 0 6px;color:#3E5372;font-size:12px;text-transform:uppercase;letter-spacing:.1em">Message</p><p style="font-size:14px;line-height:1.6;white-space:pre-wrap">${escapeHtml(enquiry.message)}</p>` : ''}
